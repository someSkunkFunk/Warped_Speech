...
%TODO: compare to warped stimuli peakrate distributions (muted code for
%now)
...
norm=true;
peakRateFile=sprintf('%s/medianStretchyRule3SegBarkPeakrate.mat',peakRateDir);
...
minInt = 0.125; %8 hz
maxInt = 0.5; % 2 hz
% filter long silences  based on sil-tol param used to generate peakrate
filter_sil=true;
if filter_sil
    sil_tol=0.75;
else
...
load(peakRateFile,'peakRateIntervals','fs','conditions');
[ogIntervals,csIntervals,stIntervals]=deal(peakRateIntervals(1,:), ...
    peakRateIntervals(2,:),peakRateIntervals(3,:));
ogIntervals=vertcat(ogIntervals{:});
csIntervals=vertcat(csIntervals{:});
stIntervals=vertcat(stIntervals{:});
% filter silent pauses
if filter_sil
    fprintf('removing pauses longer than %0.2f s\n',sil_tol)
    ogIntervals(ogIntervals>sil_tol)=[];
    csIntervals(csIntervals>sil_tol)=[];
    stIntervals(stIntervals>sil_tol)=[];
end

[ogFreqs,csFreqs,stFreqs]=deal(1./ogIntervals,1./csIntervals, ...
    1./stIntervals);
%NOTE: that long pauses will be included in intervals, just filter them out

ogIntervals=unitFactor*ogIntervals;
...
%% Find upper and lower quartiles using mathematics
upper_quartile=inverse_truncated_gamma(optParams,.75,1/minInt,1/maxInt);
lower_quartile=inverse_truncated_gamma(optParams,.25,1/minInt,1/maxInt);
fprintf('upper/lower quartiles: %0.3f/%0.3f Hz\n',upper_quartile,lower_quartile)
...
legend()
title('Gamma distribution fit to peakRate (bark env) frequencies')
hold off
...
function cdf_inverse=inverse_truncated_gamma(params,p,ubound,lbound)
% cdf_inverse=inverse_truncated_gamma(params,p,ubound,lbound)
% finds this values of truncated gamma fit with shape scale params
% (estimated with truncated a upper bound ubound and lower bound 
% ubound) such that the probability of cdf_inverse is equal to p
    shape=params(1);
    scale=params(2);
    F_ubound=gamcdf(ubound,shape,scale);
    F_lbound=gamcdf(lbound,shape,scale);
    F_trunc=p*(F_ubound-F_lbound)+F_lbound;
    cdf_inverse=gaminv(F_trunc,shape,scale);
end